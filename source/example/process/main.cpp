...
int main(int argc, char *argv[], char *envp[])
{
	...
	if (argc <= 1)
	{
		...
		(
			{
				...
				Ash::Concurrency::Process::Environment::get("UNKNOWN"),
				{ "test1", "value1" },
			}
		);

		block.set({ "test2", "hello world" }).throwOnError();
		...
		process.run({ Ash::Concurrency::Process::getCurrentName(), "argument 1", "I said \"hello world!\"", "argument\"3", "test\\path with\\spaces\\", "\\\"argument\\5\\" }, block).throwOnError();
	}
