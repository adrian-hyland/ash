		...
		namespace Encoding
		{
			namespace Utf16le
			{
				static Ash::Test::Assertion character()
				{
					...
					{
						Ash::Encoding::Utf16le::Character character(code);

						TEST_IS_EQ(character.getLength(), 2);

						TEST_IS_EQ(uint8_t(*character.at(0)), (code & 0xFF));
						TEST_IS_EQ(uint8_t(*character.at(1)), (code >> 8));
					...
					for (Ash::Unicode::Character::Value code = 0x10000; code < 0x110000; code++)
					{
						...
						TEST_IS_EQ(uint8_t(*character.at(0)), (((code - 0x10000) >> 10) & 0xFF));
						TEST_IS_EQ(uint8_t(*character.at(1)), 0xD8 + (((code - 0x10000) >> 18) & 0x03));
						TEST_IS_EQ(uint8_t(*character.at(2)), ((code - 0x10000) & 0xFF));
						TEST_IS_EQ(uint8_t(*character.at(3)), 0xDC + (((code - 0x10000) >> 8) & 0x03));
				...
				static Ash::Test::Assertion decodeNext()
				{
					...
					{
						TEST_IS_TRUE(content.append(Ash::Encoding::Utf16le::Character(character)));
					}
					...
					for (Ash::Unicode::Character::Value code = 0xD8000000; code < 0xE0000000; code++)
					{
						...
						TEST_IS_TRUE(invalidContent.set(1, code >> 24));
						TEST_IS_TRUE(invalidContent.set(2, code));
						TEST_IS_TRUE(invalidContent.set(3, code >> 8));
				...
				static Ash::Test::Assertion decodePrevious()
				{
					...
					for (Ash::Unicode::Character::Value code = 0; code <= Ash::Unicode::Character::maximum; code++)
					{
						TEST_IS_TRUE(content.append(Ash::Encoding::Utf16le::Character(code)));
					}
